...
static void calc_throttle()
{
    ...
    if (!alt_control_airspeed()) {
        ...
        } else {//                                  45              75                 45            1000            4500     
			//if (hal.rcin->read(CH_5) > 1500){
				//g.channel_throttle.servo_out = 0;
			//}else{
...
static void set_servos(void)
{
    ...
    if (control_mode == MANUAL) {
        ...
        if (g.mix_mode == 0) { // ELEVON_MIXING - Bei Nurflueger auf 1
            ...
            /*Elevon mode*/
            float ch1;
            float ch2;                                      //reverse_elevons fuer zeph = 1
        ...
        }                                                                 //reverse_ch2_elevon fuer zeph = 1
    ...
    if(control_mode < FLY_BY_WIRE_B) { // RTL = 11
        RC_Channel_aux::copy_radio_in_out(RC_Channel_aux::k_flap_auto);
	...
	/*
	if (MotorAbstellen > 0 || hal.rcin->read(CH_5) < 1500){ //wenn Fallschrim ausgelöst wird
		g.channel_throttle.radio_out = 1010;
		//hal.console->println("--> motor wurde abgestellt!");
		//hal.console->println("motor wurde ausgeschalten");
		//servo_write(CH_5, 1002); //fallschirm wird ausgelöst
	}
	*/
