...
// BEI EEPROM RESET CONFIG FILE FUER DRONE ZUREUCKLADEN

// 07.06.2013

/*------------------LOG-----------------------
 2013.06.13 -   Loiter radius wird kleiner gmeacht sobald der flieger nahe am ziel ist
 2013.06.12 -   IMPD wurde als neues logfile eingefügt für wichtige daten über den flieger
 2013.06.07 -   HEAD_MAX wurde auf 25 und WP_RADIUS_DEFAULT sowie LOITER_RADIUS_DEFAULT auf 300meter geändert
 2013.06.01 -	lm335 verbunden achtung muss mit float gelesen werden kann acuh mit g.LM335offset geeicht werden. probleme mit der loc_pos parameter
				wurden nun geändert. es muss nun so gebraucht werden wie es steht und keine andere Reihenfolge
 2013.05.31 -	release_wire_voltage wurde hizugefügt
 2013.05.29 -	static AP_HAL::AnalogSource * temp_pin1; wurde eingefuegt evtl. gibt es probleme damit???
 2013.05.28 -   portierung von 2.68 auf 2.73
 2013.03.04 -	minHeigth darf man nicht multiplizieren wenn man in einer If Schleifen ist!	ist wegen dem 16bit "problem" von define darum const verwenden
 2013.02.07 -	DRONE_BATTERY_EVENT hibzugef�gt dass nur dieser event get�tigt wird wenn es auch wirklich eine drone ist und nicht im fpv FLug!
 2013.01.31 -	fpv und release_altitude ist nun mit den g-paramters gesetzt sprich eproom wirte wird �bernommen und kann nun auch mit app gesteuert werden. standard parameter f�r fpv=0
 				standard paramter release_altitude=12000
 2013.01.30 -	failsafeCounter wurde vor read_control_switch() gesetzt damit falls EzUHF den switch auf failsafe stellt APM nicht darauf reagiert. z.B. Situation wenn im AUTO modus und die
 				Fernbedienung ausgeschalten wird will ich nicht, dass das Flugzeug in Failsafe mode vom EzUHF geht
 xxxx.xx.xx -	Ballon kann jetzt auf Zeit ausgel�st werden mit avSteigAusloese wir die Zeit welche ein ballon mit avSteigAusloese m/s hat berechnet und falls er noch nicht ausegel�st
 				hat nach dieser Zeit wird er ausgekoppelt das ganze wird ab 10km h�he gestertet
 */
...
//fuer glider
static AP_HAL::AnalogSource * temp_pin1;
//static AP_HAL::AnalogSource * release_wire_status;
...
static int32_t old_nav_pitch_cd;
...
////////////////////////NEU////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////
#define time_after_reached		5 //in sekunden (wie lange der auslösemechnaismus noch warten soll bis er den ballon abtrennt)
#define avSteigAusloese			4 //in m/s
const unsigned long minHeigth = 10000; //in meter über Meer

static uint8_t			unlock;
static uint8_t			wpNum = 1;
static uint8_t			OSDactivate = 1;
static uint8_t			switchRTL = 0;
static uint32_t			milC34;
static float			release_wire_voltage = 0;
static uint8_t			countC34send = 0;

static uint8_t			lastValueThrottle = 1;
static uint8_t			ChangePitch = 0;

uint8_t					battery_remaining = -1;
uint16_t				battery_current = -1;
static float			tempLM335K = 0;
static float			temperatureDallas = 0;

static uint8_t 			superslow_loopCounter2;
static uint8_t 			superslow_loopCounter3;
static uint8_t			slow_loopCounterDallas = 0;
static uint8_t 			ausl = 0;

static uint8_t			MotorAbstellen = 0;

static uint8_t          eventAusgeloest = 0;
static uint8_t          ledPosition;
static uint8_t 			externActivate;
static uint8_t			safeAltitudeReached;
//static long 	        release_altitude;
static int32_t			oldAlt;
static struct 	        Location loc_pos_home;
static struct 	        Location loc_pos_1;
static struct 	        Location loc_pos_2;
static struct 	        Location loc_pos_3;
static struct 	        Location loc_pos_pay;
static uint32_t			safeAltitudeReachedMillis;
...
void setup() {
  ...
  //fuer glider
  temp_pin1 = hal.analogin->channel(TEMP_PIN_1);
  //release_wire_status = hal.analogin->channel(RELEASE_WIRE_STATUS_PIN_1);
  ...
  //----------------------------------------------NEU-START--------------------------------------------------------------//
  if (g.fpv){
    //Serial2.begin(57600, 128, SERIAL_BUFSIZE);
    hal.uartB->begin(SERIAL0_BAUD, 128, SERIAL_BUFSIZE);
    pinMode(13, OUTPUT); //um Bluetooth Modul einzustellen
    pinMode(44, OUTPUT); //status LED ob Plane configuriert und GPS da!
  }
  else{	
    setupRelais();
    setupC34();
    g.curr_amp_offset = -0.016;
  }
  //setupLED();	
  //----------------------------------------------NEU-ENDE--------------------------------------------------------------//
}
...
static void medium_loop()
{
  ...
  case 1:
    ...
    intRTL(); //eigens intelligentes RTL
...
static void slow_loop()
{
  ...
  case 0:
    ...
    ownSonar();
    if (!g.fpv){
      ReleaseRelaisActivate();
	  readControlSwitches();
      //checkLED();
      if ((ausl < 2)){  
        superslow_loopCounter2++;  
      }
    }

    //setLED(); //ist aus wenn am configurieren, blink wenn noch kein GPS aber konfiguriert, ist solid wenn GPS lock und konfigueriert
    //superslow_loopCounter3++;

    bluetoothModul(); //bluetooth modul ein oder ausschalten		
    //----------------------------------------------NEU-ENDE--------------------------------------------------------------//
...
static void one_second_loop()
{
  ...
  sendOSD();
  /*
  if (slow_loopCounterDallas > 5){
	  //readControlSwitches();
	  //lm335Read();
	  //readDallas();
	  slow_loopCounterDallas = 0;
  }
  slow_loopCounterDallas++;
  */
}
...
static void update_current_flight_mode(void)
{
  if(control_mode == AUTO) {
    ...
    case GUIDED:
      ...
      calc_nav_roll();
      calc_nav_pitch();
	  //wird 50mal pro sekunde aufgerufen


	  /*
	  //-0.0000003*(25000:-1:15000).^2+0.0055*(25000:-1:15000)-20

	  if (g_gps->ground_speed < 14000 && current_loc.alt >= 1500000){ //nur wenn glider langsamer als 140m/s ist und er höher als 15000m ist
		  nav_pitch_cd = ((-0.0000003 * (current_loc.alt / 100.0)*(current_loc.alt / 100.0))+ 0.0055 * (current_loc.alt / 100.0) - 20) * 100.0;
		  nav_pitch_cd = constrain_int32(nav_pitch_cd, -9000, 0);
	  }
	  */
	  ...
	  /*
	  //Für Drone Zeph
	  //mit fernsteuerung kann ich mehr pitch gegen oder nicht, aber nur immer gegen unten also negativer pitch
	    if (hal.rcin->read(CH_7) > 1000){
			  static int32_t oldPitchit = 0;
			  oldPitchit = nav_pitch_cd;
			  nav_pitch_cd = oldPitchit + (abs(hal.rcin->read(CH_7) - 1000) / -11.1 * 100.0);  //ergibt ungefähr ein range von 0 - -9000
			  nav_pitch_cd = constrain_int32(nav_pitch_cd, -9000, 0);
			  old_nav_pitch_cd = nav_pitch_cd;
		  }
		  */
		...
		if (failsafeCounter <= 0){
			...
			if (hal.rcin->read(CH_5) > 1550 && current_loc.alt >= 300000UL){
				nav_pitch_cd = ((hal.rcin->read(CH_5) * 0.2) - 310) * 100.0;
				nav_pitch_cd = nav_pitch_cd + oldPitchit;
				ChangePitch = 1;
			}else if(hal.rcin->read(CH_5) < 1450 && current_loc.alt >= 300000UL){ //-0.1111*C5+161.11
				nav_pitch_cd = -1 * ((hal.rcin->read(CH_5) * -0.1111) + 161.11) * 100.0;
				nav_pitch_cd = nav_pitch_cd + oldPitchit;
				ChangePitch = 1;
			}else{
				ChangePitch = 0;
			}
			//nav_pitch_cd = constrain_int32(nav_pitch_cd, -9000, 9000);
			old_nav_pitch_cd = nav_pitch_cd;
		}else{
		  if (ChangePitch == 1 && current_loc.alt >= 300000UL){
			  //nav_pitch_cd = old_nav_pitch_cd;
		  }
		}
	  ...
	  /*
	  //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	  //Nur gebrauchen für Privatflieger
	  //mit fernsteuerung kann ich mehr pitch gegen oder nicht, aber nur immer gegen unten also negativer pitch
	  if (hal.rcin->read(CH_5) > 1002){
		  static int32_t oldPitchit = 0;
		  oldPitchit = nav_pitch_cd;
		  nav_pitch_cd = oldPitchit + (abs(hal.rcin->read(CH_5) - 1000) / -11.1 * 100.0);  //ergibt ungefähr ein range von 0 - -9000
		  nav_pitch_cd = constrain_int32(nav_pitch_cd, -9000, 0);
	  }
	  //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	  */
	  ...
	  /*
	  hal.console->print(nav_pitch_cd);
	  hal.console->println();
	  */

	  calc_throttle();
	  if (current_loc.alt > (g.RTL_altitude_cm + home.alt + 20000UL))
		  g.channel_throttle.servo_out = 0;


	  //Für Drone Zeph - mann kann den Throttle ausschalten aber er schlatet ihn auch selbstädig wieder
	  //ein sobal 3000müm unterschritten sind.
	  /*
	  if (failsafeCounter <= 0){
		  if (hal.rcin->read(CH_5) > 1500  && current_loc.alt >= 300000UL){
			  g.channel_throttle.servo_out = 0;
			  lastValueThrottle = 0;
		  }else{
			  lastValueThrottle = 1;
		  ...
		  if (lastValueThrottle == 0 && current_loc.alt >= 300000UL){
			  g.channel_throttle.servo_out = 0;
		  }
	  }
	  */
    ...
    case STABILIZE:
		...
		//Nur gebrauchen für Privatflieger
		if (hal.rcin->read(CH_5) > 1002){
			nav_pitch_cd = abs(hal.rcin->read(CH_5) - 1000) / -11.1 * 100.0;
			nav_pitch_cd = constrain_int32(nav_pitch_cd, -9000, 0);
		}
		//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		*/
		...
		if (hal.rcin->read(CH_5) > 1550){ //0.1*B5-155
			nav_pitch_cd = oldPitchit + ((abs(hal.rcin->read(CH_5) - 1550) * 0.1) - 155) * 100.0;
		}else if(hal.rcin->read(CH_5) < 1450){
			nav_pitch_cd = oldPitchit - ((abs(hal.rcin->read(CH_5) - 1450) * 0.1) - 155) * 100.0;
		}
		...
		*/
		...
		/*
		hal.console->print(nav_pitch_cd); //ergibt ungefähr ein range von 0 - -9000
		hal.console->println();
		*/
