...
static void Log_Write_Nav_Tuning()
{
    struct log_Nav_Tuning pkt = {
        ...
        altitude_error_cm   : (int32_t)altitude_error_cm,
...
struct PACKED log_Important_Data {
    LOG_PACKET_HEADER;
    uint32_t wpDistance;
    int16_t throttle_out;
    int16_t roll_out;
	...
	int16_t barometer_temp;
	int32_t barometer_pressure;
};

static void Log_Write_Important_Data()
{
    struct log_Important_Data pkt = {
        LOG_PACKET_HEADER_INIT(LOG_IMPD_MSG),
		wpDistance				: wp_distance,
		throttle_out			: (int16_t)g.channel_throttle.servo_out,
        roll_out				: (int16_t)g.channel_roll.radio_out,
		pitch_out				: (int16_t)g.channel_pitch.radio_out,
		nav_roll_cd				: (int16_t)nav_roll_cd,
		nav_pitch_cd			: (int16_t)nav_pitch_cd,
		CH5Value			    : (int16_t)hal.rcin->read(CH_5),
		crGPS					: g_gps->velocity_down(),
		battery_voltage         : (int16_t)(battery_voltage1 * 100.0),
        current_amps            : (int16_t)(current_amps1 * 100.0),
		barometer_temp			: (int16_t)(barometer.get_temperature()/10.0),
		barometer_pressure		: (int32_t)barometer.get_pressure()
    };
    DataFlash.WriteBlock(&pkt, sizeof(pkt));
}
...
static const struct LogStructure log_structure[] PROGMEM = {
    ...
    { LOG_NTUN_MSG, sizeof(log_Nav_Tuning),         
      "NTUN", "CICCec",     "Yaw,WpDist,TargBrg,NavBrg,AltErr,Arspd" },
	...
	{ LOG_IMPD_MSG, sizeof(log_Important_Data),     
      "IMPD", "Ihcccccfcche",    "wpD,ThrOut,ROut,POut,NRoll,NPitch,CH5,crGPS,V,Cur,barT,barP" },
};
...
static void Log_Read(uint16_t log_num, int16_t start_page, int16_t end_page)
{
	...
	//Dieses File findet man im LogFile.cpp im Dataflash ordner in den libraries
