...
static int8_t	setup_release_altitude				(uint8_t argc, const Menu::arg *argv);
...
#endif
    {"erase",                       setup_erase},
    {"release",			setup_release_altitude},
};
...
static int8_t
setup_release_altitude(uint8_t argc, const Menu::arg *argv)
{
	int c;
	int d;
	long burstAlt = 0;
	cliSerial->printf_P(PSTR("\nType 'S13450' um die release hoehe auf 13450m zu setzten, 'E' um die burst altitude zu lesen: "));
	...
	if (('e' != c) && ('E' != c)){  
                  ...
                  if (d != 13 && d > 0){ //Enter
                    burstAlt *= 10;
                    burstAlt = burstAlt + (d - 48);
                  }else if (d == 13){
                    //Serial.printf_P(PSTR("\nnew release Altitude: \n"));
                    //Serial.print("$r,");
                    //Serial.print(burstAlt);
                    //Serial.println(",");
					cliSerial->println();
					g.release_altitude = burstAlt * 100;
                    //eeprom_write_dword((uint32_t *)4080, burstAlt);
                    return 0;
                  }
                }
        }else{
			//Serial.printf_P(PSTR("\nAktuelle release Altitude: \n"));
			cliSerial->printf_P(PSTR("\n$release,"));
			cliSerial->println(g.release_altitude / 100);
			//Serial.print((long)eeprom_read_dword((uint32_t*)4080));			
			cliSerial->printf_P(PSTR("\n"));
			cliSerial->println();
        }
