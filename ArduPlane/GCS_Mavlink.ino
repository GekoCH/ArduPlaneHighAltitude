...
static NOINLINE void send_extended_status1(mavlink_channel_t chan, uint16_t packet_drops)
{
	...
	//static AP_AnalogSource_Arduino release_wire_status_pin(release_wire_status_pin_1);
    //uint16_t RAW_release_wire_status = release_wire_status_pin.read_average();
	/*
	uint8_t release_wire_status_val = 0;
	release_wire_voltage = (release_wire_status->voltage_average()*(RELEASE_WIRE_R1+RELEASE_WIRE_R2)/RELEASE_WIRE_R2); 
	if (release_wire_voltage >= 8)
		release_wire_status_val = 1;
	else
		release_wire_status_val = 0;
			*/
    ...
    mavlink_msg_sys_status_send(
        ...
        0, // 
        0, // 		
        0, //release_wire_status, // errors_count1
		(g.release_altitude / 100), // errors_count2
		g.fpv, // errors_count3
		tempLM335K); // errors_count4 

		/*
        Serial.println();
        static AP_AnalogSource_Arduino current_pin(CURRENT_PIN_1);
        Serial.println(current_pin.read_average());
        Serial.println(CURR_AMPS_OFFSET);
        //Serial.println(g.volt_div_ratio);
        //Serial.println(g.input_voltage);
        Serial.println(battery_voltage1);
...
//----------------------------------------------NEU-START--------------------------------------------------------------//            
            case MAV_CMD_RELAY_RELEASE:
                externActivate = 1;
                ReleaseRelaisActivate();
                result = MAV_RESULT_ACCEPTED;
                break;   
   
             case MAV_CMD_RELAY_RELEASE_PROHIBIT:
                g.release_altitude = 40000 * 100; //wird auf 40km gesetzt (in cm) so wird der automatische Release sicher nie ausgelöst :)
                result = MAV_RESULT_ACCEPTED;
                break;    
                
            case MAV_CMD_RELAY_LED_POSITION:
                if (ledPosition)
					ledPosition = 0;
                else
					ledPosition = 1;
			...
			case MAV_CMD_SET_HOME_ALT:
				home.lat 	= loc_pos_home.lat;				// Lon * 10^7
				home.lng 	= loc_pos_home.lng;				// Lat * 10^7
				home.alt 	= loc_pos_home.alt;             // cm				
				result = MAV_RESULT_ACCEPTED;
				break;      
			
				/*
			case MAV_CMD_SET_RELEASE_ALT:
				g.release_altitude = packet.param1 * 100; //in cm
				if (packet.param1 < minHeigth)
					g.release_altitude = minHeigth * 100;
				eeprom_write_dword((uint32_t *)4080, packet.param1);
				result = MAV_RESULT_ACCEPTED;
				break;
				*/
			case MAV_CMD_SET_MIN_GND_SPEED:
				g.min_gndspeed_cm.set(packet.param1 * 100); //in cm/s
				g.min_gndspeed_cm.save(); //savets in eeprom
				result = MAV_RESULT_ACCEPTED;
				break;  				
